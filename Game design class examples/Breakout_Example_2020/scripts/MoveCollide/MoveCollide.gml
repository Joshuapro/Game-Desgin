...
function MoveCollide(){
	...
	x_remainder = full_xspd % 1; //saves decimal
	...
	if (xspd_floored != 0){
		for (var i=0; i < abs(xspd_floored); i++) {
			if(place_meeting(x+sign(xspd_floored),y,obj_collidable)){
				...
				if(hitThing.object_index == obj_block){
					Obj_screenshaker.shake_timer = .25*room_speed
					Obj_screenshaker.shake_amt = 5
					spawpart(x,y,200)
				}else if (hitThing.object_index == obj_paddle){
					Obj_screenshaker.shake_timer = .2*room_speed
					Obj_screenshaker.shake_amt = 3
				}
